...
class NvIndicator:
    def __init__(self):
        ...
        for gpu in xml.gpu:
            ...
            # memory usage - update 3
            item = Gtk.MenuItem()
            mem_used = str(gpu.fb_memory_usage[0].used[0])
            mem_total = str(gpu.fb_memory_usage[0].total[0])
            item.set_label(
                "MEM Usage: {}M / {}M ({}%)".format(
                    mem_used[:mem_used.index(' ')],
                    mem_total[:mem_total.index(' ')],
